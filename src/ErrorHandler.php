...
class ErrorHandler
{
    ...
    public function handleException($e)
    {
        ...
        $this->logFile(
            ...
            ['exception' => $e, 'error_code'=>$e->getCode(), 'file'=>$e->getFile(),'line'=>$e->getLine(), 'function'=>$e->getTrace()[0]['function'], 'class'=>$e->getTrace()[0]['class']],
            Logger::ERROR
        );
    ...
    public function handleError($code, $message, $file = '', $line = 0, $context = [])
    {
        ...
        if (!$this->hasFatalErrorHandler || !in_array($code, self::$fatalErrors, true)) {
            $this->logFile(
                ...
                ['error_code' => $code, 'message' => $message, 'file' => $file, 'line' => $line],
                Logger::CRITICAL
            );
        } else {
    ...
    public function handleFatalError()
    {
        ...
        if ($lastError && in_array($lastError['type'], self::$fatalErrors, true)) {
            $this->logFile(
                ...
                ['error_code' => $lastError['type'], 'message' => $lastError['message'], 'file' => $lastError['file'], 'line' => $lastError['line'], 'function' => 'cannotget_function', 'class'=>'cannotget_class'],
                Logger::ALERT
                );
    ...
    private function logFile(string $message = '', array $context = [], int $level = Logger::INFO):bool
    {
        if($level == Logger::CRITICAL) {
            $t = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 3);
            $context['file']  = $t[1]['file'];
            $context['line']  = $t[1]['line'];
            $context['function']  = $t[2]['function'];
            $context['class']  = $t[2]['class'];
        }
        ...
        return $this->logger->oalog($message, $context, $level);
    }
}
